...
class AdvancedAIPredictor:
    ...
    def _calculate_features(self, symbol):
        ...
        if prices is None:
            # ลองหา symbol ในรูปแบบอื่น
            alt_symbols = [symbol.replace('/', ''), symbol.replace('USDT', 'USD'), 
                          symbol.replace('USD', 'USDT'), symbol.replace('/', 'USD')]
            for alt in alt_symbols:
                if alt in self.history:
                    prices = self.history[alt]
                    break
        
        if prices is None or len(prices) < self.min_history_size:
        if not prices or len(prices) < self.min_history_size:
            print(f"⚠️ Not enough historical data for {symbol} prediction ({len(prices) if prices else 0}/{self.min_history_size} points)")
            return None
